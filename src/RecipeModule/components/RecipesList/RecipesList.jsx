...
import { USER_RECIPE_URLS } from "../../../services/api/apiURLs";
...
export default function RecipesList() {
  ...
  const handleShow = (res) => {
    // هنا المشكلة - res هو id فقط، ليس recipe object
    // لذلك نبحث عن الوصفة في القائمة
    const recipe = recipesList.find((recipe) => recipe.id === res);
    if (recipe) {
      setRecId(recipe.id);
  ...
  let addToFav = async (id) => {
    try {
      await axiosInstance.post(USER_RECIPE_URLS.CREATE_FAV, { recipeId: id });
    ...
    <>
      ...
      <div className="container-fluid py-4">
                      ...
                      {openId === recipe.id && (
                        ...
                        >
                          <button
                            ...
                            onClick={() => handleAction("view", recipe.id)}
                          >
                          ...
                          {loginData?.userGroup == "SystemUser" ? (
                            ...
                            >
                              <i className="fa-solid fa-heart text-success me-2"></i>{" "}
                              Fav
                            </button>
                          ) : (
                            <div>
                              <button
                                className="dropdown-item py-2"
                                onClick={() => handleAction("edit", recipe.id)}
                              >
        ...
        <div className="d-flex gap-2 mt-4 justify-content-center">
          ...
          <span className="align-self-center mx-2 small">
